...
func switch_the_deck() {
	...
	originalDeck := current_deck // make a bkup copy
	for {
		...
		fmt.Printf("    \"quiz\"    %d cards in the Japan Quiz deck\n\n", quiz_japan_len)
		...
		if current_deck != "all" &&
			...
			return
		} else if current_deck == "quiz" {
			fmt.Printf("=Enter deck: nov, comp, mast, guru, jap - Here%s:> %s", colorCyan, colorReset)
			_, _ = fmt.Scan(&current_deck) // current_deck is a global var.
			...
			for {
				if current_deck == "comp" {
					...
					resetAllLogs()
					return
				} else if current_deck == "jap" {
					current_deck = "quiz_japan"
					deck_len = quiz_japan_len
					resetAllLogs()
					return
				} else {
			...
			for {
				...
				// todo ::: not sure about "rs" being listed here ???????
